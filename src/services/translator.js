...
async function translateSegments(segments, sourceLang = 'auto') {
  ...
  for (const batch of batches) {
    ...
    const prompt = `Translate the following ${sourceLang==='auto'?'non-English':''} lines to English. Preserve numbering and do not add or remove lines. Only output the translated lines numbered the same.\n\n${enumerated}`;
    ...
    batch.forEach((s, i) => {
      out.push({ ...s, text: map.get(i) || s.text });
    });
  }
  return out;
}
