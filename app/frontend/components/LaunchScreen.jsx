...
export default function LaunchScreen({ onComplete }) {
  ...
  useEffect(() => {
    console.log('🎬 LaunchScreen mounted, creating timeline...');
    
    // Prevent double animations by checking if already animating
    let isAnimating = true;
    
    const tl = gsap.timeline({ 
      onComplete: () => {
        if (!isAnimating || hasCompletedRef.current) return; // Guard against double calls
        console.log('🎬 LaunchScreen timeline completed, calling onComplete callback');
        isAnimating = false;
        hasCompletedRef.current = true;
        // Add a small delay to ensure the animation completes properly
        setTimeout(() => {
          if (onComplete && !window.__edulensLaunchCompleted) {
            onComplete();
          }
        }, 100);
      }
    ...
    console.log('🎬 LaunchScreen timeline created with total duration:', tl.duration());
    
    // Cleanup function to prevent double animations
    return () => {
      console.log('🎬 LaunchScreen unmounting, killing timeline...');
      isAnimating = false;
      tl.kill(); // Kill the timeline if component unmounts early
    };
  }, [onComplete]);
