...
export default function useFocusMode() {
    ...
    async (timerDuration = 1800000, options = {}) => {
      ...
      try {
        ...
        try {
          ...
          if (hc) {
            console.log('AI bridge health verified');
          }
        } catch (e) {
          console.warn('Failed to verify AI bridge health:', e);
        }
  ...
  const getFocusStatus = useCallback(async () => {
    try {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        console.warn('Focus API returned non-JSON response, using fallback');
        // Return fallback data
        return { active: false, remaining: 0, session: null, isFocusMode: false };
      }
      
      if (!response.ok) {
        console.warn(`Focus API returned ${response.status}, using fallback`);
        return { active: false, remaining: 0, session: null, isFocusMode: false };
      }
      
      const result = await response.json();
      setSession(result.session);
      setIsFocusMode(result.isFocusMode || result.active || false);
      return result;
    } catch (err) {
  ...
  useEffect(() => {
    getFocusStatus().catch((e) => {
      console.warn('Failed to get focus status:', e);
    });
  }, []);
