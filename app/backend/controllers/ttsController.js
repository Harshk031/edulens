...
// Import TTS queue functions (assuming they're also converted to CommonJS)
let ttsQueue;
try {
  ttsQueue = require('../jobs/ttsQueue.js');
} catch (err) {
  console.warn('TTS queue not available:', err.message);
  // Create fallback functions
  ttsQueue = {
    enqueueTTSJob: async (data) => `fallback_job_${Date.now()}`,
    getJob: (jobId) => ({ status: 'not_implemented' }),
    watchJob: (jobId) => ({ status: 'not_implemented' })
  };
}

const CACHE_DIR = process.env.TTS_CACHE_DIR || path.join(dirname, '..', 'cache', 'tts');
